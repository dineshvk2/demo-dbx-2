...
spark.sql('DROP TABLE IF EXISTS dk_demo_temps_table')
temps.write.saveAsTable('dk_demo_temps_table')

# Query the table on the cluster, returning rows
# where the airport code is not BLI and the date is later
# than 2021-04-01. Group the results and order by high
# temperature in descending order.
df_temps = spark.sql("SELECT * FROM dk_demo_temps_table " \
...
spark.sql('DROP TABLE dk_demo_temps_table')
